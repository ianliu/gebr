			      ...
			      GParamSpec * param_spec)
{
	...
	case ENUM_OPTION:{
			...
			gtk_list_store_clear(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store);
...
static void enum_option_edit_class_init(EnumOptionEditClass * klass)
{
	...
	/* virtual */
	enum_option_edit_class = GEBR_GUI_SEQUENCE_EDIT_CLASS(klass);
...
static void __enum_option_edit_remove(EnumOptionEdit * enum_option_edit, GtkTreeIter * iter)
{
	...
	gtk_tree_model_get(GTK_TREE_MODEL(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store), iter,
			   2, &sequence, -1);

	gebr_geoxml_sequence_remove(sequence);
	gtk_list_store_remove(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store, iter);
			...
			GtkTreeViewDropPosition drop_position)
{
	...
	gtk_tree_model_get(GTK_TREE_MODEL(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store), iter,
			   2, &sequence, -1);
	gtk_tree_model_get(GTK_TREE_MODEL(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store), position,
			   2, &position_sequence, -1);

	if (drop_position == GTK_TREE_VIEW_DROP_AFTER) {
		gebr_geoxml_sequence_move_after(sequence, position_sequence);
		gtk_list_store_move_after(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store, iter, position);
	} else {
		gebr_geoxml_sequence_move_before(sequence, position_sequence);
		gtk_list_store_move_before(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store, iter, position);
	}
...
static void __enum_option_edit_move_top(EnumOptionEdit * enum_option_edit, GtkTreeIter * iter)
{
	...
	gtk_list_store_move_after(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store, iter, NULL);
...
static void __enum_option_edit_move_bottom(EnumOptionEdit * enum_option_edit, GtkTreeIter * iter)
{
	...
	gtk_list_store_move_before(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store, iter, NULL);
...
static GtkWidget *__enum_option_edit_create_tree_view(EnumOptionEdit * enum_option_edit)
{
	...
	tree_view =
	    gtk_tree_view_new_with_model(GTK_TREE_MODEL(GEBR_GUI_SEQUENCE_EDIT(enum_option_edit)->list_store));
