...
static gboolean enum_from_string(GType type, const gchar * string, gint * enum_value)
{
	GEnumClass *eclass;
	GEnumValue *ev;
	gchar *endptr;
...
static gboolean flags_from_string(GType type, const gchar * string, gint * flags_value)
{
	GFlagsClass *fclass;
...
static gboolean json_deserialize_pspec(GValue * value, GParamSpec * pspec, JsonNode * node)
{
	GValue node_value = { 0, };
		...
		case G_TYPE_ENUM:
			{
				...
				if (G_VALUE_HOLDS(&node_value, G_TYPE_INT)) {
					...
					retval = enum_from_string(G_VALUE_TYPE(value),
								  g_value_get_string(&node_value), &enum_value);
				}
		...
		case G_TYPE_FLAGS:
			{
				...
				if (G_VALUE_HOLDS(&node_value, G_TYPE_INT)) {
					...
					retval = flags_from_string(G_VALUE_TYPE(value),
								   g_value_get_string(&node_value), &flags_value);
				}
...
static JsonNode *json_serialize_pspec(const GValue * real_value, GParamSpec * pspec)
{
				       ...
				       GValue * value, GParamSpec * pspec, JsonNode * property_node)
{
	...
	return iface->deserialize_property(serializable, property_name, value, pspec, property_node);
}

static gboolean
json_serializable_real_deserialize(JsonSerializable * serializable,
				   const gchar * name, GValue * value, GParamSpec * pspec, JsonNode * node)
{
	return json_deserialize_pspec(value, pspec, node);
}

static JsonNode *json_serializable_real_serialize(JsonSerializable * serializable,
						  const gchar * name, const GValue * value, GParamSpec * pspec)
{
	return json_serialize_pspec(value, pspec);
}

static void json_serializable_base_init(gpointer g_class, gpointer data)
{
...
GType json_serializable_get_type(void)
{
	...
	if (!iface_type)
		iface_type =
		    g_type_register_static_simple(G_TYPE_INTERFACE, "JsonSerializable",
						  ...
						  json_serializable_base_init, 0, NULL, 0);
...
GObject *json_construct_gobject(GType gtype, const gchar * data, gsize length, GError ** error)
{
	...
	if (root == NULL || JSON_NODE_TYPE(root) != JSON_NODE_OBJECT) {
		g_set_error(error, JSON_PARSER_ERROR,
			    ...
			    "Expecting a JSON object, but the root node " "is of type `%s'", json_node_type_name(root));
		g_object_unref(parser);
		return NULL;
	}
	...
	for (l = members; l != NULL; l = l->next) {
		...
		if (deserialize_property) {
			res = iface->deserialize_property(serializable, pspec->name, &value, pspec, val);
		}
...
gchar *json_serialize_gobject(GObject * gobject, gsize * length)
{
	...
	for (i = 0; i < n_pspecs; i++) {
		...
		if (serialize_property) {
			node = iface->serialize_property(serializable, pspec->name, &value, pspec);
		}
	...
	gen = g_object_new(JSON_TYPE_GENERATOR, "root", root, "pretty", TRUE, "indent", 2, NULL);
