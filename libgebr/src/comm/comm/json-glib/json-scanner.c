...
static JsonScannerConfig json_scanner_config_template = {
	...
	("_" G_CSET_a_2_z G_CSET_A_2_Z) /* cset_identifier_first */ ,
	(G_CSET_DIGITS "-_" G_CSET_a_2_z G_CSET_A_2_Z) /* cset_identifier_nth */ ,
...
static const gchar json_symbol_names[] = "true\0" "false\0" "null\0" "var\0";
...
/* --- prototypes --- */
static gboolean json_scanner_key_equal(gconstpointer v1, gconstpointer v2);
static guint json_scanner_key_hash(gconstpointer v);

static inline
    JsonScannerKey * json_scanner_lookup_internal(JsonScanner * scanner, guint scope_id, const gchar * symbol);
static void json_scanner_get_token_ll(JsonScanner * scanner,
				      GTokenType * token_p, GTokenValue * value_p, guint * line_p, guint * position_p);
static void json_scanner_get_token_i(JsonScanner * scanner,
				     GTokenType * token_p, GTokenValue * value_p, guint * line_p, guint * position_p);

static guchar json_scanner_peek_next_char(JsonScanner * scanner);
static guchar json_scanner_get_char(JsonScanner * scanner, guint * line_p, guint * position_p);
static gunichar json_scanner_get_unichar(JsonScanner * scanner, guint * line_p, guint * position_p);
static void json_scanner_msg_handler(JsonScanner * scanner, gchar * message, gboolean is_error);

/* --- functions --- */
static inline gint json_scanner_char_2_num(guchar c, guchar base)
{
	if (c >= '0' && c <= '9')
		c -= '0';
	else if (c >= 'A' && c <= 'Z')
...
JsonScanner *json_scanner_new(void)
{
	...
	scanner->symbol_table = g_hash_table_new(json_scanner_key_hash, json_scanner_key_equal);
	scanner->input_fd = -1;
	scanner->text = NULL;
...
static inline void json_scanner_free_value(GTokenType * token_p, GTokenValue * value_p)
{
	switch (*token_p) {
	case G_TOKEN_STRING:
...
static void json_scanner_destroy_symbol_table_entry(gpointer _key, gpointer _value, gpointer _data)
{
...
void json_scanner_destroy(JsonScanner * scanner)
{
	...
	g_hash_table_foreach(scanner->symbol_table, json_scanner_destroy_symbol_table_entry, NULL);
...
static void json_scanner_msg_handler(JsonScanner * scanner, gchar * message, gboolean is_error)
{
	...
	g_fprintf(stderr, "%s:%d: ", scanner->input_name ? scanner->input_name : "<memory>", scanner->line);
	if (is_error)
...
void json_scanner_error(JsonScanner * scanner, const gchar * format, ...)
{
	g_return_if_fail(scanner != NULL);
...
void json_scanner_warn(JsonScanner * scanner, const gchar * format, ...)
{
	g_return_if_fail(scanner != NULL);
	g_return_if_fail(format != NULL);
...
static inline JsonScannerKey *json_scanner_lookup_internal(JsonScanner * scanner, guint scope_id, const gchar * symbol)
{
...
void json_scanner_scope_add_symbol(JsonScanner * scanner, guint scope_id, const gchar * symbol, gpointer value)
{
	JsonScannerKey *key;
...
void json_scanner_scope_remove_symbol(JsonScanner * scanner, guint scope_id, const gchar * symbol)
{
	JsonScannerKey *key;
...
gpointer json_scanner_lookup_symbol(JsonScanner * scanner, const gchar * symbol)
{
	JsonScannerKey *key;
	guint scope_id;
...
gpointer json_scanner_scope_lookup_symbol(JsonScanner * scanner, guint scope_id, const gchar * symbol)
{
	JsonScannerKey *key;
...
guint json_scanner_set_scope(JsonScanner * scanner, guint scope_id)
{
	guint old_scope_id;
...
static void json_scanner_foreach_internal(gpointer _key, gpointer _value, gpointer _user_data)
{
...
void json_scanner_scope_foreach_symbol(JsonScanner * scanner, guint scope_id, GHFunc func, gpointer user_data)
{
	...
	g_hash_table_foreach(scanner->symbol_table, json_scanner_foreach_internal, &closure);
}
...
GTokenType json_scanner_peek_next_token(JsonScanner * scanner)
{
	...
	if (scanner->next_token == G_TOKEN_NONE) {
		...
		json_scanner_get_token_i(scanner,
					 &scanner->next_token,
					 &scanner->next_value, &scanner->next_line, &scanner->next_position);
	}
...
GTokenType json_scanner_get_next_token(JsonScanner * scanner)
{
	...
	if (scanner->next_token != G_TOKEN_NONE) {
		...
		json_scanner_get_token_i(scanner, &scanner->token, &scanner->value, &scanner->line, &scanner->position);
...
void json_scanner_input_file(JsonScanner * scanner, gint input_fd)
{
	g_return_if_fail(scanner != NULL);
...
void json_scanner_input_text(JsonScanner * scanner, const gchar * text, guint text_len)
{
	g_return_if_fail(scanner != NULL);
	if (text_len)
			 ...
			 const gchar * symbol_spec, const gchar * symbol_name, const gchar * message, gint is_error)
{
	...
	case G_TOKEN_SYMBOL:
		if (expected_token == G_TOKEN_SYMBOL ||
		    (scanner->config->symbol_2_token && expected_token > G_TOKEN_LAST))
			print_unexp = FALSE;
		if (symbol_name)
			g_snprintf(token_string, token_string_len,
				   "%s%s `%s'", print_unexp ? "" : "invalid ", symbol_spec, symbol_name);
		else
			g_snprintf(token_string, token_string_len, "%s%s", print_unexp ? "" : "invalid ", symbol_spec);
		break;
	...
	case G_TOKEN_IDENTIFIER:
	case G_TOKEN_IDENTIFIER_NULL:
		if (expected_token == G_TOKEN_IDENTIFIER || expected_token == G_TOKEN_IDENTIFIER_NULL)
			print_unexp = FALSE;
		g_snprintf(token_string, token_string_len,
			   "%s%s `%s'",
			   print_unexp ? "" : "invalid ",
			   identifier_spec, scanner->token == G_TOKEN_IDENTIFIER ? scanner->value.v_string : "null");
		break;
	...
	case G_TOKEN_HEX:
		if (scanner->config->store_int64)
			g_snprintf(token_string, token_string_len, "number `%" G_GUINT64_FORMAT "'",
	...
	case G_TOKEN_STRING:
		...
		g_snprintf(token_string, token_string_len,
			   ...
			   print_unexp ? "" : "invalid ",
			   scanner->value.v_string[0] == 0 ? "empty " : "", scanner->value.v_string);
	...
	case G_TOKEN_SYMBOL:
		need_valid = (scanner->token == G_TOKEN_SYMBOL ||
			      (scanner->config->symbol_2_token && scanner->token > G_TOKEN_LAST));
		g_snprintf(expected_string, expected_string_len, "%s%s", need_valid ? "valid " : "", symbol_spec);
	...
	case G_TOKEN_STRING:
		g_snprintf(expected_string,
			   expected_string_len, "%sstring constant", scanner->token == G_TOKEN_STRING ? "valid " : "");
		break;
	case G_TOKEN_IDENTIFIER:
	case G_TOKEN_IDENTIFIER_NULL:
		need_valid = (scanner->token == G_TOKEN_IDENTIFIER_NULL || scanner->token == G_TOKEN_IDENTIFIER);
		g_snprintf(expected_string, expected_string_len, "%s%s", need_valid ? "valid " : "", identifier_spec);
		break;
	case G_TOKEN_COMMENT_SINGLE:
		tstring = "single-line";
	...
	if (expected_token == G_TOKEN_ERROR) {
		...
		if (print_unexp)
			msg_handler(scanner,
				    ...
				    token_string, expected_string, message_prefix, message);
		else
			msg_handler(scanner,
				    "%s, expected %s%s%s", token_string, expected_string, message_prefix, message);
	}
			  ...
			  GTokenType * token_p, GTokenValue * value_p, guint * line_p, guint * position_p)
{
	...
	do {			/* while (ch != 0) */
		...
		case '/':
			if (!config->scan_comment_multi || json_scanner_peek_next_char(scanner) != '*')
				goto default_case;
					...
					if (ch == '\\') {
						...
						case 'u':
							...
							if (is_hex_digit(fchar)) {
								...
								ucs =
								    json_scanner_get_unichar(scanner, line_p,
											     position_p);
						...
						case '7':
							...
							if (fchar >= '0' && fchar <= '7') {
								...
								fchar = json_scanner_peek_next_char(scanner);
								if (fchar >= '0' && fchar <= '7') {
									ch = json_scanner_get_char(scanner, line_p,
												   position_p);
									i = i * 8 + ch - '0';
								}
							}
 ...
 number_parsing:
			{
				...
				do {	/* while (in_number) */
					...
					if (json_scanner_char_2_num(ch, 36) >= 0 ||
					    (config->scan_float && ch == '.') || (is_E && (ch == '+' || ch == '-'))) {
						ch = json_scanner_get_char(scanner, line_p, position_p);
						...
						case '.':
							if (token != G_TOKEN_INT && token != G_TOKEN_OCTAL) {
								value.v_error =
								    token ==
								    G_TOKEN_FLOAT ? G_ERR_FLOAT_MALFORMED :
								    G_ERR_FLOAT_RADIX;
								token = G_TOKEN_ERROR;
								in_number = FALSE;
							} else {
						...
						case 'E':
							if ((token != G_TOKEN_HEX && !config->scan_float) ||
							    (token != G_TOKEN_HEX &&
							     token != G_TOKEN_OCTAL &&
							     token != G_TOKEN_FLOAT && token != G_TOKEN_INT)) {
								token = G_TOKEN_ERROR;
								value.v_error = G_ERR_NON_DIGIT_IN_CONST;
								in_number = FALSE;
							} else {
 ...
 identifier_precedence:
					...
					if (config->cset_identifier_nth && ch &&
					    strchr(config->cset_identifier_nth, json_scanner_peek_next_char(scanner))) {
		...
		if (token == G_TOKEN_IDENTIFIER && config->scan_identifier_NULL && strlen(value.v_identifier) == 4) {
			...
			if (scanner->config->case_sensitive) {
				if (value.v_identifier[0] == null_upper[0] &&
				    ...
				    value.v_identifier[2] == null_upper[2] && value.v_identifier[3] == null_upper[3])
