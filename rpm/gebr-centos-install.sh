...
print_usage()
{
	...
	echo "  TMP_DIR               The temporary directory in which packages will be"
	echo "                        downloaded and builded. Default is ./gebr-tmp-dir."
	echo
	echo "  GEBR_VERSION          Version of GeBR source to be downloaded em built."
	echo "                        Default is 0.14.0."
	echo
	...
	echo "  --destdir=INSTALL_DIR Installation directory."
	echo
...
GEBR_VERSION=${GEBR_VERSION-0.15.9}
...
TEMP=`getopt -o hc: --longoptions prefix:,destdir:,cores:,enable-debug,help \
	-n "$SCRIPT_NAME" -- "$@"`
...
export PATH=$DEST_DIR/$PREFIX_DIR/bin:$PATH
export LD_LIBRARY_PATH=$DEST_DIR/$PREFIX_DIR/lib:$LD_LIBRARY_PATH
export PKG_CONFIG_PATH=$DEST_DIR/$PREFIX_DIR/lib/pkgconfig:$PKG_CONFIG_PATH

TMP_DIR=${TMP_DIR-gebr-tmp-dir}
mkdir -p $TMP_DIR
[ -d ../gebrproject-$GEBR_VERSION ] && rm -rf $TMP_DIR/gebrproject-$GEBR_VERSION && \
...
# icu4c-4_2_1-src
	...
	&& cd icu/source \
	&& make DESTDIR=$DEST_DIR install && cd ../.. || exit 1
...
> 	*dest = '\\0';
EOF
	[ $? -eq 0 ] && ./configure --prefix=$PREFIX_DIR $DEBUG)) \
	&& cd libXt-0.1.5 \
	&& make DESTDIR=$DEST_DIR -j$CORES install && cd .. || exit 1
fi
...
EOF
	[ $? -eq 0 ] && ./configure --prefix=$PREFIX_DIR $DEBUG --without-gnome)) \
	&& cd libsoup-2.25.91 \
	&& make DESTDIR=$DEST_DIR -j$CORES install && cd .. || exit 1
...
> 
EOF
		[ $? -eq 0 ] && make DESTDIR=$DEST_DIR install -j$CORES && cd .. || exit 1
	else
		make DESTDIR=$DEST_DIR install -j$CORES && cd .. || exit 1
	fi
...
if [ -d rpm ]; then
	...
	for dir in libgebr gebrd gebr debr; do
		cd $dir && ./configure --prefix=$PREFIX_DIR $DEBUG \
		&& make DESTDIR=$DEST_DIR install -j$CORES && cd ..
	done
	cd ..
fi
...
#Save binaries
cd $DEST_DIR/$PREFIX_DIR/bin
GEBR_BINS=`ls debr gebr*`
mkdir -p $DEST_DIR/$PREFIX_DIR/lib/libgebr/bin
mv $GEBR_BINS $DEST_DIR/$PREFIX_DIR/lib/libgebr/bin
cd - > /dev/null

#Tidy yp
if [ "$PREFIX_DIR" = "/opt/gebrproject" -a "$DEBUG" != "--enable-debug" ]; then
	rm $DEST_DIR/$PREFIX_DIR/bin/*
	strip $DEST_DIR/$PREFIX_DIR/lib/libgebr/bin/*
	find $DEST_DIR/$PREFIX_DIR/lib -iname "*\.a" | xargs rm
	find $DEST_DIR/$PREFIX_DIR/lib -iname "*\.la" | xargs rm
	find $DEST_DIR/$PREFIX_DIR/lib -iname "*\.so" | xargs strip
	rm -r $DEST_DIR/$PREFIX_DIR/sbin
	rm -r $DEST_DIR/$PREFIX_DIR/man
	rm -r $DEST_DIR/$PREFIX_DIR/include
	rm -r $DEST_DIR/$PREFIX_DIR/share/gtk-doc
	find $DEST_DIR/$PREFIX_DIR/share/locale/* -maxdepth 0 \( ! -iname "pt*" ! -iname "en*" \) | xargs rm -r
	find $DEST_DIR/$PREFIX_DIR/share/doc/* -maxdepth 0 \( ! -iname "gebr*" \) | xargs rm -r
fi
