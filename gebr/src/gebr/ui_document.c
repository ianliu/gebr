...
struct dict_edit_data * data);
static void
dict_edit_load_iter(struct dict_edit_data * data, GtkTreeIter * iter, GebrGeoXmlParameter * parameter);
static void
...
static void
dict_edit_new_parameter_iter(struct dict_edit_data * data, GebrGeoXmlObject * object, GtkTreeIter * iter);
static void
dict_edit_append_add_parameter(struct dict_edit_data * data, GtkTreeIter * document_iter);
static GtkMenu *
...
static const gchar *
document_get_name_from_type(GebrGeoXmlDocument * document, gboolean upper);
	...
	case GTK_RESPONSE_OK: {
		...
		case GEBR_GEOXML_DOCUMENT_TYPE_FLOW:
			...
			gtk_list_store_set(gebr.ui_flow_browse->store, &iter,
				FB_TITLE, gebr_geoxml_document_get_title(document),
				-1);
...
document_dict_edit_setup_ui(void)
{
	...
	for (int i = 0; data->documents[i] != NULL; ++i) {
		...
		gtk_tree_store_set(GTK_TREE_STORE(data->tree_model), &document_iter,
			...
			DICT_EDIT_KEYWORD_EDITABLE, FALSE,
	...
	for (int i = 0; data->documents[i] != NULL; ++i) {
		...
		i_parameter = gebr_geoxml_parameters_get_first_parameter(
			gebr_geoxml_document_get_dict_parameters(data->documents[i]));
...
struct dict_edit_data * data)
{
	...
	if (is_add_parameter) {
		parameter = gebr_geoxml_parameters_append_parameter(
			gebr_geoxml_document_get_dict_parameters(data->current_document),
...
dict_edit_load_iter(struct dict_edit_data * data, GtkTreeIter * iter, GebrGeoXmlParameter * parameter)
{
	...
	gtk_tree_store_set(GTK_TREE_STORE(data->tree_model), iter,
		...
		DICT_EDIT_KEYWORD_IMAGE, "",
		DICT_EDIT_KEYWORD, gebr_geoxml_program_parameter_get_keyword(
			GEBR_GEOXML_PROGRAM_PARAMETER(parameter)),
		DICT_EDIT_VALUE, gebr_geoxml_program_parameter_get_first_value(
...
dict_edit_check_duplicates(struct dict_edit_data * data, GebrGeoXmlDocument * document, const gchar * keyword)
{
	...
	for (int i = 0; data->documents[i] != document; ++i) {
		...
		for (; i_parameter != NULL; gebr_geoxml_sequence_next(&i_parameter)) {
			...
			pixbuf = (strcmp(gebr_geoxml_program_parameter_get_keyword(
			GEBR_GEOXML_PROGRAM_PARAMETER(i_parameter)), keyword))
...
const gchar * keyword, gboolean show_error)
{
	...
	for (; i_parameter != NULL; gebr_geoxml_sequence_next(&i_parameter )) {
		...
		if (i_parameter == GEBR_GEOXML_SEQUENCE(parameter))
			continue;
		if (strcmp(gebr_geoxml_program_parameter_get_keyword(
		GEBR_GEOXML_PROGRAM_PARAMETER(i_parameter)), keyword))
			continue;
		if (show_error)
