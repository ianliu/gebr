...
void gebrd_queues_add_job_to(const gchar * queue, GebrdJob * job)
{
	...
	job_queue = g_hash_table_lookup(gebrd->user->queues, queue);
	if (!job_queue) {
		job_queue = gebrd_job_queue_new(queue);
		g_hash_table_insert(gebrd->user->queues, g_strdup(queue), job_queue);
	}
...
void gebrd_queues_remove(const gchar * queue)
{
	...
	job_queue = g_hash_table_lookup(gebrd->user->queues, queue);
	g_hash_table_remove(gebrd->user->queues, queue);
...
void gebrd_queues_rename(const gchar * queue, const gchar *newname)
{
	...
	g_hash_table_insert(gebrd->user->queues, g_strdup(newname), job_queue);
}
...
gchar * gebrd_queues_get_names()
{
	...
	g_hash_table_foreach(gebrd->user->queues, (GHFunc)gebrd_queues_iter, string);
	if (string->len > 0)
